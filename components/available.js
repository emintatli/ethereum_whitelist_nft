    ...
    if(data?.status=="ok"){
		...
		const minAbi=[{
		  ...
		  const web3 = new Web3(provider);
   		  const contract = new web3.eth.Contract(minAbi,contractAddress);
		  const tx=await contract.methods.awardItemWL(userWallet,amountToMint,data.sign.messageHash,data.sign.signature).send({from:userWallet,value:amountToMint*Web3.utils.toWei(nftPrice, 'ether')});
		  if(tx){
