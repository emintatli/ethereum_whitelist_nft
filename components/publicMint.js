...
export default function PublicMint(props) {
  ...
  const wlmintHandler = async () => {
	setLoading(true);
    const req = await fetch("/api/signForPublic", {
      method: "POST",
      headers: {
		  ...
		  },
      {
        ...
        "type": "function"
		  ...
		  const web3 = new Web3(provider);
   		  const contract = new web3.eth.Contract(minAbi,contractAddress);
		  const tx=await contract.methods.awardItemPB(userWallet,amountToMint,data.sign.messageHash,data.sign.signature).send({from:userWallet,value:amountToMint*Web3.utils.toWei(nftPrice, 'ether')});
		  if(tx){
